...
module.exports = new Command({
    ...
    async run(bot,message,args,db) {
        ...
        await message.reply({content: `${user.tag} a été banni par ${message.user === undefined ? message.author.tag : message.user.tag} pour la raison ${reason}`, components: [btn]}).then(async msg => {
            ...
            const date = Date.now()
            let sql = `INSERT INTO bans (userID, authorID, banID, guildID, reason, date, time) VALUES ('${user.id}', '${message.user === undefined ? message.author.id : message.user.id}', '${ID}', '${message.guildId}', '${reason}', '${date}', 'Définitif')`
            ...
            collector.on("collect", async button => {
                ...
                if(button.customId === "unban") {
                    console.log("boutton unban appuyé")
